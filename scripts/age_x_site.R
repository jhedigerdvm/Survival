#survival model looking at the interaction between age and birthsite
...
for (i in 1:3){ #site
  ...
  age.site.beta[i,j] ~ dnorm(0, 0.01) #priors for interaction between age and site
}
}
      ...
      for (t in (f[i]+1):h[i]){ 
        # State process
            ...
            logit(phi[i,t-1]) <- int + age.beta[ageclass[i,t-1]] + site.beta[bs[i]] + eps.capyear[capyear[t-1]] + 
                                          age.site.beta[ageclass[i,t-1]*bs[i]]
        # Observation process
...
#       survival[i,j] <- exp(int+ age.beta[j] + site.beta[i] + age.site.beta[i*j])/ 
#                                   (1 + exp(int+ age.beta[j] + site.beta[i]+ age.site.beta[i,j]))
...
inits <- function(){list(int = rnorm(1, 0, 1), z = z.init, age.beta = c(NA, rnorm(13,0,1)), site.beta = c(NA, rnorm(2, 0, 1)),
                         eps.capyear = c(NA, rnorm(13,0,1)), sigma = runif(1,0,10), age.site.beta=rnorm(42,0,1))} #

parameters <- c('int', 'age.beta', 'site.beta', 'p', 'survival', 'eps.capyear', 'site_diff','age.site.beta')
