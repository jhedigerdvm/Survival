...
NA_indices <- matrix(NA, nrow=489, ncol=15)
...
for (u in 1:489){                 #for u individuals 1 to 489
...
# Bundle data
jags.data <- list(ch = ch, f = f, h = h, nind = nrow(ch), nocc = ncol(ch), weight = weight, 
                  occasions=occasions, NA_indices=NA_indices)#capyear=capyear, birthyear = birthyear  bs = bs,weight.sim = weight.sim,ageclass = ageclass
