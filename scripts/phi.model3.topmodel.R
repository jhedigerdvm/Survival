...
for (i in 1:nind){
   ...
   #derived parameters
      for ( i in 1:2){ #site
        ...
        # for (j in 2:15) { #difference between 2 and 1 and 3 and 2, etc
        #     phi.diff[1,j] <- phi.age[1, j] - phi.age[1, j-1]
        #   }
...
parameters <- c('int', 'beta1', 'beta2', 'beta3', 'phi.age', 'phi.diff', 'eps1')
...
# print(model3)
# write.csv(model3$summary, './output/model3topmodel.csv')
...
ggsave('./figures/phi.age.topmodel.jpg', phi.plot, width = 8, height = 8)
