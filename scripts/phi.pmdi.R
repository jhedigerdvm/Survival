#survival analysis with pmdi data
...
write.csv(phi.weight$summary, './output/phi.final.weight.pmdi.csv', row.names = T)
...
morpho.sim.usc <- (weight.sim * sd(data$weightkg, na.rm = T)) + mean(data$weightkg, na.rm = T)
...
phi.plot<- gather %>%
  ...
  scale_fill_viridis_d(option = 'turbo', alpha = .2, labels = c("CONTROL", "TREATMENT", "TGT") ) + #this allowed me to opacify the ribbon but not the line
  scale_color_viridis_d(option = 'turbo', labels = c("CONTROL", "TREATMENT", "TGT"))+ #color of line but no opacification
  labs(x = "SPRING PMDI", y = "ANNUAL SURVIVAL PROBABILITY", title = "")+
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.PMDI.jpg', phi.plot, width = 10, height = 10)
...
phi.plot<- gather %>% filter(!age == '11') %>% 
  ...
  scale_fill_viridis_d(option = 'turbo', alpha = .2, labels = c("CONTROL", "DMP", "TGT") ) + #this allowed me to opacify the ribbon but not the line
  scale_color_viridis_d(option = 'turbo', labels = c("CONTROL", "DMP", "TGT"))+ #color of line but no opacification
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.AGE.3sites.jpg', phi.plot, width = 10, height = 10)
...
#priors
  ...
  beta3 ~ dnorm(0,0.001)  #spring pmdi
  beta4 ~ dlnorm(0, 0.01)    # morpho beta 
...
# Bundle data
jags.data <- list(h = h, ch = ch, f = f, nind = nrow(ch), ageclass = ageclass, rain = pmdi.spring.sc,
                  bs = bs, morpho.sim = antler.sim, rain.sim = pmdi.spring.sc.sim,
...
write.csv(phi.antlers$summary, './output/phi.final.antlers.pmdi.csv', row.names = T)
