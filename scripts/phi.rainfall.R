...
# Specify model in JAGS language
set.seed(100)
sink("cjs-rain-site-ran-age-id-by.jags")
cat("
model {
...
for (u in 2:14) {
  eps.ageclass[u] ~ dnorm(sigma, tau)
}
  sigma ~ dunif(0,10) #standard dev
  tau <- 1/(sigma*sigma) #precision = 1/variance

for (u in 2:489) {
  eps.id[u] ~ dnorm(sigma, tau)
}
   ...
   # Define latent state at first capture, we know for sure the animal is alive
      ...
      for (t in (f[i]+1):h[i]){ 
        # State process
            ...
            mu1[i,t] <- phi[i,t-1] * z[i,t-1]  #t-1 because we are looking ahead to see if they survived from 1 to 2 based upon them being alive at 2
            logit(phi[i,t-1]) <- int  + site.beta[bs[i]] + rain.beta*rain[i,t-1]  + rain.site.beta[bs[i]]*rain[i,t-1] + 
                                      eps.ageclass[ageclass[i,t-1]] + eps.id[id[i]] + eps.birth[birthyear[i]]
...
# Bundle data
jags.data <- list(h = h, ch = ch, f = f, nind = nrow(ch),  bs = bs, rain = annual.rainfall, rain.sim = rain.sim, birthyear = birthyear,
...
inits <- function(){list(int = rnorm(1,0,1), z = z.init, rain.beta = rnorm(1, 0, 1), sigma = runif(1,0,1),
                         site.beta = c(NA, rnorm(2,0,1)), rain.site.beta = c(NA, rnorm(2,0,1)), eps.birth = c(NA, rnorm(13,0,1)), 
...
# Call JAGS from R (BRT 3 min)
cjs.rain.site.ran.age.id.by <- jagsUI(jags.data, inits, parameters, "cjs-rain-site-ran-age-id-by.jags", n.chains = nc, 
                                n.thin = nt, n.iter = ni, n.burnin = nb, parallel = FALSE)

print(cjs.rain.site.ran.age.id.by)
# write.csv(cjs.rain.site.ran.age.id$summary, './output/rain.ran.age.id.csv')

#create a tibble of the posterior draws
gather2<- cjs.rain.site.ran.age.id.by %>% gather_draws(survival[rain,site]) #this creates a dataframe in long format with indexing
...
phi.plot.ran.age<- gather2 %>%
  ...
  scale_fill_viridis_d(alpha = .2) + #this allowed me to opacify the ribbon but not the line
  scale_color_viridis_d()+ #color of line but no opacification
  labs(x = "RAINFALL (in)", y = "ANNUAL SURVIVAL PROBABILITY", title = "Phi ~ int + site + rain + rain*site + (1|ageclass) + (1|id) + (1|birthyear)")+
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(),
...
ggsave('./figures/phi_rain_site_ran_age_id_by.jpg', phi.plot.ran.age, width = 15, height = 10)
