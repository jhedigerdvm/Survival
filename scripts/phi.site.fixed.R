#survival analysis using site as a fixed effect
...
#   
# for ( u in 1:3) { 
#   beta4[u] ~ dnorm(0, 0.001)#interaction between birth year rain and site
#   beta5[u] ~ dnorm(0, 0.001) #site effect
# }
# 
# for (u in 1:nind){      #prior for missing morphometrics
...
#       for (j in 1:3){ #site, beta5
# 
#       survival[i,j] <- exp( beta3*rain.sim[i]  + beta5[j] + beta4[j]*rain.sim[i] )/
#                             (1 + exp( beta3*rain.sim[i]  + beta5[j] + beta4[j]*rain.sim[i]))
#       
#     } # for j
#     } # for l
#     
