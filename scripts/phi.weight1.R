...
parameters <- c('int', 'bs.beta', 'weight.beta', 'bs.weight.beta', 'age.beta', 
                'eps.capyear', 'survival')# 'site_diff', 'surv_diff',, 'survival',
...
#find first row for 2nd rain value
first_idx <- which(gather$weight == 2)[1] # 1800 values of rain 1
...
vector <- numeric(0)
weight.sim2 <- for (i in weight.sim1) {
  rep_i <- rep(i, times = 1800)   # change to accomodate first idx
  vector <- c(vector,rep_i)
