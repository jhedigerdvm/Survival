#survival analysis using site as a fixed effect and two groups east yana and dmp
...
data <- data %>% filter(!bs == 'wy') #filter only dmp and east yana
...
bs <- as.numeric(factor(id.bs.by$bs)) # 1 = dmp, 2 = ey
# bs.2 <- bs
...
h <- replace(h, is.infinite(h), ncol(ch)) #change to equal number of columns/years
h
f-h #check for zero
...
for (u in 1:15){  #prior for year effect
  eps2[u] ~ dnorm(0,tau)
}
...
inits <- function(){list(#int = rnorm(1,0,1), 
  ...
  beta4 = rlnorm(2,0,1),#morpho and site interaction
  beta5 = rnorm(2, 0, 1), #site effect
  eps2 = rnorm(15, 0, 1) #capture year random effect
)
}
...
write.csv(phi.weight$summary, './output/phi.weighttxsite.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  scale_fill_viridis_d(option = 'turbo', alpha = .2, labels = c("DMP", "CONTROL") ) + #this allowed me to opacify the ribbon but not the line
...
phi.plot
ggsave('./figures/phi.weightxsite.2sites.jpg', phi.plot, width = 10, height = 10)
...
parameters <- c( 'beta1','beta2', 'beta3', 'beta4', 'beta5', 'eps2', 'surv_diff', 'survival')#'int', 'beta2',  
...
write.csv(phi.antlers$summary, './output/phi.antlersxsite.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.antlersxsite.2sites.jpg', phi.plot, width = 10, height = 10)
...
#priors
  ...
  beta2 ~ dlnorm(0, 0.1)    # morpho beta continuous
  beta3 ~ dnorm(0,0.001)  #birth year rain beta continuous
  beta4[2] ~ dnorm(0, 0.001)#interaction between age and site
  beta5[2] ~ dnorm(0, 0.001) #site effect
...
for (u in 2:15){  #prior for year effect
  eps2[u] ~ dnorm(0,tau)
}
...
inits <- function(){list(
  ...
  beta2 = rlnorm(1, 0, 1),#morpho beta
  beta3 = rnorm(1, 0, 1), # rain beta
  beta4 = c(NA, rnorm(1,0,1)),#age and site interaction
  beta5 = c(NA, rnorm(1,0,1)), #site effect
  eps2 = c(NA, rnorm(14, 0, 1)) #random effect for capture year
...
print(phi.age)
MCMCtrace(phi.age)
write.csv(phi.age$summary, './output/phi.agexsite.2site.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  stat_lineribbon(.width = 0.95)+ #statline ribbon takes posterior estimates and calculates CRI
  scale_fill_viridis_d(option = 'turbo', alpha = .2, labels = c("DMP", "CONTROL") ) + #this allowed me to opacify the ribbon but not the line
  scale_color_viridis_d(option = 'turbo', labels = c("DMP", "CONTROL"))+ #color of line but no opacification
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.agexsite.2site.jpg', phi.plot, width = 10, height = 10)
...
inits <- function(){list(
  ...
  beta2 = rlnorm(1, 0, 1),#morpho beta
  beta3 = rnorm(1, 0, 1), # rain beta
  beta4 = c(NA, rnorm(1,0,1)),#rain and site interaction
  beta5 = c(NA, rnorm(1,0,1)), #site effect
  eps2 = c(NA, rnorm(14, 0, 1))
)
}
...
print(phi.rain)
MCMCtrace(phi.rain)
write.csv(phi.rain$summary, './output/phi.rainxsite.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.byrainxsite.twosites.jpg', phi.plot, width = 10, height = 10)



# ---- Model5: phi ~ capture year rain x site interaction, fixed effect ----
...
print(phi.rain)
MCMCtrace(phi.rain)
write.csv(phi.rain$summary, './output/phi.cyrainxsite.twosites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.cyrainxsite.twosites.jpg', phi.plot, width = 10, height = 10)
...
write.csv(phi.priorbyrain$summary, './output/phi.byraincarryover.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.bypriorrainxsite.2sites.jpg', phi.plot, width = 10, height = 10)
...
write.csv(phi.priorcyrain$summary, './output/phi.priorcyrainxsite.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.priorcyrainxsite.2sites.jpg', phi.plot, width = 10, height = 10)
...
#priors
  ...
  beta1 ~ dnorm(0, 0.001)  # age beta continuous
  beta2 ~ dlnorm(0, 0.1)    # morpho beta continuous
  beta4[2] ~ dnorm(0, 0.001)#interaction between rain and site
  beta5[2] ~ dnorm(0, 0.001) #site effect
...
print(phi.rain)
MCMCtrace(phi.rain)
write.csv(phi.rain$summary, './output/phi.cyrainxsite.group.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.cyrainxsite.cat.2sites.jpg', phi.plot, width = 10, height = 10)
...
print(phi.rain)
MCMCtrace(phi.rain)
write.csv(phi.rain$summary, './output/phi.byrainxsite.group.2sites.csv', row.names = T)
...
phi.plot<- gather %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA)) #transparent plot bg)
phi.plot
ggsave('./figures/phi.byrainxsite.cat.2sites.jpg', phi.plot, width = 10, height = 10)
