...
bs <- unique(data[, c("animal_id", "bs",'birth_year')])
...
#survival as a function of antlers (continuous), age (1-15), site (all 3), age x site, random effect of capture year
# Specify model in JAGS language
set.seed(100)
...
#derived parameters
    for (j in 1:10){ #ageclass
      for (k in 1:3){ #site
      survival[j,k] <- exp(int + age.beta[j] + bs.beta[k] )/
                            (1 + exp(int + age.beta[j] + bs.beta[k] ))
      } #for k
    }   #for j
