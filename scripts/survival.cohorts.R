#cohort effects, if you were born in 2017, you had pretty solid surv probabilities 
#modeled age and cohort as a fixed effect, am i losing power with separating them by such specific ages, 
#would it be better to break down into fawn, juvenile, mature, senescent?
...
# Bundle data
jags.data <- list(h = h, CH = CH, f = f, nind = nrow(CH), ageclass = ageclass, capyear = capyear)#,, bs = bs, 

# Initial values
inits <- function(){list(int = rnorm(1, 0, 1), z = z.init, cohort.beta = c(NA, rnorm(13, 0, 1)),
                         eps.capyear = c(NA, rnorm(13,0,1)))}
  
  # (age.beta = c(NA, rnorm(13,0,1))
  #                        , sigma = runif(1,0,10))} #

parameters <- c('int', 'cohort.beta', 'survival', 'eps.capyear')
# 'survival', ''age.beta',site_diff' 'survival',, 'site_diff','eps.capyear' 'int','site.beta','int', 'age.beta', 'site.beta', 'p', 'survival', 'eps.capyear', 'site_diff'
