...
#load data 
data<-read.csv('./cleaned/data_cleaned21.csv', header = T)
...
#get rid of rows of all zeroes
sum <- apply(CH, 1, sum) #change to second capture occasion CH
CH <- CH[-which(sum==0),]
dim(CH)

#create a vector with 1 for treatment, 2 for control, 3 for tgt
bs<- as.numeric(factor(data$birth_site[-which(sum==0)]))
unique(bs) 
bs  #dmp is 1, e yana 2, w yana 3

#get version of data that has rows removed
data2 <- data[-which(sum==0),]
...
for (i in 1:dim(ageclass)[1]){
  ageclass[i,1] <- 2007 - data2$birth_year[i]  +1
  ageclass[i,2] <- 2007 - data2$birth_year[i]  +2
  ageclass[i,3] <- 2007 - data2$birth_year[i]  +3
  ageclass[i,4] <- 2007 - data2$birth_year[i]  +4
  ageclass[i,5] <- 2007 - data2$birth_year[i] +5
  ageclass[i,6] <- 2007 - data2$birth_year[i] +6
  ageclass[i,7] <- 2007 - data2$birth_year[i] +7
  ageclass[i,8] <- 2007 - data2$birth_year[i] +8
  ageclass[i,9] <- 2007 - data2$birth_year[i] +9
  ageclass[i,10] <- 2007 - data2$birth_year[i] +10
  ageclass[i,11] <- 2007 - data2$birth_year[i] +11
  ageclass[i,12] <- 2007 - data2$birth_year[i] +12
  ageclass[i,13] <- 2007 - data2$birth_year[i] +13
  ageclass[i,14] <- 2007 - data2$birth_year[i] +14
  ageclass[i,15] <- 2007 - data2$birth_year[i] +15}

ageclass

#Make any zeros or negatives NA
ageclass[ageclass<=1] <- NA #removing first age class from analysis
ageclass<- as.matrix(ageclass)

#get rid of 2007
ageclass <- ageclass[,-1]

#new f bc we have 495 indivs now
f <- apply(CH, 1, get.first) #change to second capture occasion CH
...
for (u in 2:13){   #there are 14 ages and therefore 13 survival intervals (2-15)                          
...
# 
# for(capyear in 1:13){                               ##ERROR WITH DIMENSION MISMATCH
